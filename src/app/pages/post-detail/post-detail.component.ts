  ...
  ngOnInit(): void {
    this.route.paramMap.subscribe(params => {
      ...
      if (this.post) {
        ...
        // Get travel posts for the travel section

        this.travelPosts = this.allPosts
        .filter(p => (p.category === 'Travel' || p.category === 'travel') && p.id !== this.post.id)
        .map(p => ({
            id: p.id,
            title: p.title,
            excerpt: p.content.substring(0, 100) + '...',
        ...
        // Get business posts for the business section

        this.businessPosts = this.allPosts
        .filter(p => (p.category === 'Business' || p.category === 'business') && p.id !== this.post.id)
        .map(p => ({
            id: p.id,
            title: p.title,
            excerpt: p.content.substring(0, 100) + '...',
